...
// MPU6050 using I2C communication
...
int16_t AcXH, AcXL; // ACCEL_XOUT_H, ACCEL_XOUT_L
...
void loop() {
  ...
  // request 2 bytes Acceleration
  Wire.requestFrom(MPU_addr, 2, true); // true: stop sending message after the request, false: restart
  
  AcXH=Wire.read(); // 0x3B (ACCEL_XOUT_H) & 0x3C (ACCEL_XOUT_L)
  AcXL=Wire.read(); 
  /*
  ...
  */
  ...
  Serial.print("AcXH = "); Serial.println(AcXH, BIN);
  Serial.print("AcXL = "); Serial.println(AcXL, BIN);
  /**
